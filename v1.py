...
# ----------------------------
def draw_wrapped_text(img, text, x, y, max_width, color):
    words = text.split(" ")
    line = ""
    for word in words:
        test = line + word + " "
        (w, _), _ = cv2.getTextSize(
            test, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2
        )
        if w > max_width:
...
def draw_dashboard(canvas, cam_w):
    ...
    y = draw_wrapped_text(
        canvas,
        f"Payment: {dashboard['payment']}",
        x, y, DASHBOARD_WIDTH - 40,
        (255,255,255)
    )

    cv2.putText(canvas, f"Cash Collected: {dashboard['cash']}", (x, y),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2)
    y += 35

    cv2.putText(canvas, f"Total Cash: INR {total_cash}", (x, y),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2)
    y += 35
    ...
    y += 30
    cv2.putText(canvas, f"Manual Approved: {stats['manual_approved']}", (x, y),
    ...
    if is_valid_plate(plate):
        if plate != last_plate or now - last_time > COOLDOWN:
            ...
            if "APPROVED" in decision:
                ...
                dashboard["payment"] = "Unable to process payment due to detected issues in vehicle"
                dashboard["gate"] = "CLOSED"
                dashboard["cash"] = "NO"
                stats["rejected"] += 1
                play_sound("rejected")
